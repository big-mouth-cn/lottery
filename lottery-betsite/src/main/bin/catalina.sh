...
SERVER_JAR=`ls ../lib/lottery-betsite*.jar`
...
exec "${JAVA_HOME}/bin/java" ${JAVA_OPTS} -server -Xms${HEAP_MEMORY} -Xmx${HEAP_MEMORY} -XX:PermSize=${PERM_MEMORY} -XX:MaxPermSize=${PERM_MEMORY} -XX:+HeapDumpOnOutOfMemoryError -jar ${SERVER_JAR}